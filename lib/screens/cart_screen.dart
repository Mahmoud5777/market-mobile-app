...
class _CartScreenState extends State<CartScreen> {
  ...
  Future<void> _updateQuantity(CartItemModel item, int newQuantity) async {
    ...
    try {
      await context.read<CartProvider>().updateQuantity(itemId: item.id, quantity: newQuantity, token: token);
  ...
  Future<void> _removeItem(CartItemModel item) async {
    final token = context.read<AuthProvider>().token;
    if (token == null) return;
    await context.read<CartProvider>().removeItem(itemId: item.id, token: token);
  }
          ...
          : cart.items.isEmpty
              ? Center(
                  child: Column(
                    ...
                    children: [
                      Icon(Icons.remove_shopping_cart, size: 64, color: kTextLightColor),
                      const SizedBox(height: 12),
                      Text('Ton panier est vide', style: TextStyle(color: kTextLightColor, fontSize: 16)),
                    ],
                  ),
                )
              : Column(
                                  ...
                                  Expanded(
                                    child: Column(
                                      ...
                                      children: [
                                        ...
                                        Text('\$${item.product.price.toStringAsFixed(2)} / unité',
                          ...
                          children: [
                            ...
                            SizedBox(
                              ...
                              child: ElevatedButton(
                                ...
                                child: const Text('Passer la commande', style: TextStyle(fontSize: 16)),
                              ),
                            ),
                          ],
                        ),
